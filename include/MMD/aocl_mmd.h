...
#define AOCL_MMD_VERSION_STRING "2024.2"
...
typedef enum {
    ...
    *  data port, use aocl_mmd_hostchannel_get_buffer() directly.
    */
...
/**
 *  Shared allocation migration destinations
 */
typedef enum {
 ...
 *
 *  @param  requested_info_id: A value from the aocl_mmd_offline_info_t enum
 *  @param  param_value_size: Size of the param_value field in bytes. This should
 ...
 *  @param  param_value: Pointer to the variable that will receive the returned info
 *  @param  param_size_ret: Receives the number of bytes of data actually returned
 ...
 *  info.
 *
 *  @param  name: Open the board with this name (provided as a C-style string,
 *          i.e. NUL terminated ASCII.)
 *  @return the non-negative integer handle for the board, otherwise a negative
 *          value to indicate error. Upon receiving the error, the OpenCL runtime
 *          will proceed to open other known devices, hence the MMD mustn't
 ...
 *  set
 *
 *  @param  fn: The callback function to invoke when a kernel interrupt occurs
 *  @param  user_data: The data that should be passed to fn when it is called.
 *
 *  @return 0 if successful, negative on error
 */
AOCL_MMD_CALL int aocl_mmd_set_interrupt_handler(
      int handle,
 ...
 *  set
 *
 *  @param  fn: The callback function to invoke when a device interrupt occurs
 *  @param  user_data: The data that should be passed to fn when it is called.
 *  @return 0 if successful, negative on error
 */
AOCL_MMD_CALL int aocl_mmd_set_device_interrupt_handler(
      int handle,
 ...
 *
 *   @param  fn: The callback function to invoke when a status update is to be
 *           performed.
 *   @param  user_data: The data that should be passed to fn when it is called.
 *   @return 0 if successful, negative on error
 */
AOCL_MMD_CALL int aocl_mmd_set_status_handler(
      int handle,
 ...
 *
 *  @return non-zero if the yield function performed useful work such as
 ...
 *
 *  @param  op: The operation object used to track this operations progress
 *  @param  len: The size in bytes to transfer
 *  @param  src: The host buffer being read from
 *  @param  dst: The host buffer being written to
 *  @param  mmd_interface: The handle to the interface being accessed. E.g. To
 ...
 *  @param  offset/src_offset/dst_offset: The byte offset within the interface that
 *          the transfer will begin at.
 *  @return 0 if the operation launch was successful, and negative otherwise.
 */
 ...
 *
 *  Provide host with pointer to data buffer they can access to write or
 *  read from kernel, along with space or data available in the buffer
 *  in bytes.
 *
 *  @param  channel: The handle to the channel to get the data buffer for
 *  @param  buffer_size: The address that this call will write the amount of
 *          space or data that's available in the buffer,
 *          depending on direction of the channel, in bytes
 *  @param  status: The address that this call will write to for result of this
 *          call. Value will be 0 for success, and negative otherwise
 *  @return The pointer to the data buffer that host can write to or read from.
 *          NULL if the status is negative.
 */
 ...
 *  OPTIONAL FUNCTION
 *
 *  Host Channel get sideband signal buffer operation
 *  Provide host with pointer to sideband signal buffer associated with
 *  data buffer as obtained from the get buffer operation, corresponding
 *  to the given port ID, along with space or data available in the buffer
 *  in bytes.
 *
 *  @param  channel: The handle to the channel to get the sideband signal buffer
 *          for
 *  @param  port_id: The ID to identify which sideband signal buffer to return
 *  @param  buffer_size: The address that this call will write the amount of
 *          space or data that's available in the buffer,
 *          depending on direction of the channel, in bytes
 *  @param  status: The address that this call will write to for result of this
 *          call. Value will be 0 for success, and negative otherwise
 *  @return The pointer to the sideband buffer that host can write to or read
 *          from. NULL if the status is negative.
 */
AOCL_MMD_CALL void *aocl_mmd_hostchannel_get_sideband_buffer(
      int handle,
      int channel,
      aocl_mmd_hostchannel_port_id_t port_id,
      size_t *buffer_size,
      int *status);
 ...
 *
 *  @param  channel: The handle to the channel that user is acknowledging
 *  @param  send_size: The size in bytes that the user is acknowledging
 *  @param  status: The address that this call will write to for result of this
 *          call. Value will be 0 for success, and negative otherwise
 *  @return The return value is equal to send_size if send_size was less than or
 ...
 *  the one before.
 *
 *  @param  user_data: The binary contents of the fpga.bin file created during
 *          Quartus II compilation.
 *  @param  size: The size in bytes of user_data
 *  @param  program_mode: Bit field for programming attributes. See
 *          aocl_mmd_program_mode_t definition
 *  @return The new non-negative integer handle for the board, otherwise a
 *          negative value to indicate error.
 */
AOCL_MMD_CALL int aocl_mmd_program(
      int handle,
      void * user_data,
      size_t size,
      aocl_mmd_program_mode_t program_mode) WEAK;
 ...
 *  @return to or read from.alid pointer, on error NULL
 */
AOCL_MMD_CALL void* aocl_mmd_host_alloc(
      int* handles,
      size_t num_devices,
      size_t size,
      size_t alignment,
 ...
 */
AOCL_MMD_CALL int aocl_mmd_shared_migrate(
      int handle,
      void* shared_ptr,
      size_t size,
